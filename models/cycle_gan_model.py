...
class CycleGANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_identity', type=float, default=0.5, 
                                help='use identity mapping. Setting lambda_identity other than 0 has \
                                an effect of scaling the weight of the identity mapping loss. \
    ...
    def initialize(self, opt):
        ...
        self.netG_B = networks.define_G(opt.output_nc, opt.input_nc, opt.ngf, opt.netG, opt.norm_g,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids, 
                                        opt.no_scale, opt.spectral_norm)

        if self.isTrain:
            use_sigmoid = opt.no_lsgan
            self.netD_A = networks.define_D(opt.output_nc, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.norm_d, use_sigmoid, opt.init_type, 
                                            opt.init_gain, self.gpu_ids, opt.spectral_norm)
    ...
    def backward_G(self):
        ...
        elif self.opt.loss == 'wasserstein':
            # GAN loss D_A(G_A(A))
            self.loss_G_A = - (self.netD_A(self.revise_B)).mean()
            # GAN loss D_B(G_B(B))
        ...
        self.loss_G = (self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B 
                       + self.loss_idt_A + self.loss_idt_B + self.loss_style + self.loss_style_A)
